...
[1, 2, 3].forEach(num => num * 2).filter(num => num > 2) // undefined

// map is chainable
[1, 2, 3].map(num => num * 2).filter(num => num > 2) // [4, 6]
```
...
```tsx
const animals = ['dog', 'cat', 'bird'];
const filteredAnimals = animals.map(animal => animal.toUpperCase()).filter(animal => animal !== 'dog');
```
