...
Let's talk about the normal workflow of requesting resources.
...
Although HTML and CSS are different resources, when the browser parses the HTML, and encounters any inline CSS or external CSS, it will request the CSS file.

Browser also needs to parse the CSS file, and build the **CSSOM tree**.

Here is the flow look like:

```json
Parse HTML ─────────────────────────────────────► Complete DOM parsing
     │
